...
export default function Home() {
  ...
  const sharedThemes = fullReading?.combined.agreements.map((theme) => THEME_LABELS[theme]) ?? [];
  const actionGuide = fullReading
    ? [...fullReading.combined.complementarySignals, ...fullReading.combined.tensions].map((item) => item.explanation).slice(0, 4)
    : [];
  ...
  function submitProfile(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setPalmState(createEmptyPalmStepState());
  ...
  function changePalmSide(side: PalmSide) {
    setPalmState((current) => selectPalmSide(current, side));
  }

  function continueFromPalm() {
  ...
  async function generateNarrative() {
    ...
    try {
      const result = await requestNarrative(reading, question, profile);
  ...
  return <main>
    ...
    {step === "palm" && <PalmStep state={palmState} onSelectSide={changePalmSide} onBack={() => changeStep("tarot")} onContinue={continueFromPalm}/>}
    ...
    {step === "review" && profile && <section className="review-section flow-stage">
      ...
      <div className="review-grid">
        ...
        <ReviewItem label="Palm" value={`${palmInputSummary(palmState)} (현재 분석 제외)`} onEdit={() => changeStep("palm")}/>
      </div>
      <div className="generate-panel"><p>현재는 사주, 이름 개인화, 질문, 타로를 사용해 결과를 만듭니다. 손금은 다음 Phase에서 추가됩니다.</p><button type="button" onClick={generateReading}>리딩 생성하기 <span>✦</span></button></div>
    </section>}

    {resultReady && fullReading && profile && nameResult && <>
      <section className="integrated-hero" id="integrated-reading">
        <p className="eyebrow">TODAY&apos;S GYEOL</p><h2>오늘의 結</h2>
        <p className="integrated-headline">{sharedThemes.length > 0 ? `${profile.name}님의 리딩에서는 '${sharedThemes.slice(0, 2).join(" · ")}'이라는 주제가 반복해서 나타납니다.` : `${profile.name}님이 적어 둔 질문을 중심으로, 현재 선택의 조건을 차분히 살펴보세요.`}</p>
      ...
      <section className="palm-result-placeholder">
        <div className="section-heading"><p className="eyebrow">05 · PALM</p><h2>손에 남은 결</h2><p>왼손·오른손 입력 화면은 준비되었지만 아직 손바닥 사진을 수집하거나 PalmResult를 생성하지 않습니다.</p></div>
        <div className="empty-result"><span>NOT ANALYZED</span><p>카메라 촬영, 사진 선택, 권한 처리와 미리보기는 Phase 3에서 연결하고, 이미지 최적화와 mock Palm Analyzer는 Phase 4에서 연결합니다.</p></div>
      </section>

      <section className="combined"><p className="eyebrow">06 · INTEGRATED READING</p><h2>네 가지 해석이 만나는 자리</h2><p className="combined-intro">현재는 계산과 해석이 완료된 사주와 타로만 비교합니다. 이름은 개인화에만 사용했고 손금은 아직 포함하지 않았습니다.</p><div className="signal-grid"><article><span>공통적으로 나타나는 메시지</span>{fullReading.combined.agreements.length ? <ul>{fullReading.combined.agreements.map((theme) => <li key={theme}>{THEME_LABELS[theme]}</li>)}</ul> : <p>이번 리딩에서는 뚜렷하게 겹치는 주제가 없습니다.</p>}</article><article><span>함께 확인하면 좋은 점</span>{fullReading.combined.complementarySignals.length ? <ul>{fullReading.combined.complementarySignals.map((item) => <li key={item.relationId}>{item.explanation}</li>)}</ul> : <p>별도로 덧붙일 조언은 없습니다.</p>}</article><article><span>서로 다른 방향을 가리키는 점</span>{fullReading.combined.tensions.length ? <ul>{fullReading.combined.tensions.map((item) => <li key={item.relationId}>{item.explanation}</li>)}</ul> : <p>서로 충돌하는 흐름은 두드러지지 않습니다.</p>}</article></div></section>
      ...
      <section className="action-guide-section">
        ...
        <ol>{(actionGuide.length ? actionGuide : ["지금 결정할 것과 조금 더 지켜볼 것을 나누어 적어 보세요.", "결정 전에 확인해야 할 현실 정보를 한 가지 정해 보세요."]).map((item, index) => <li key={item}><span>{String(index + 1).padStart(2, "0")}</span><p>{item}</p></li>)}</ol>
      </section>

      <section className="journal-placeholder">
        <div className="section-heading"><p className="eyebrow">08 · JOURNAL NOTE</p><h2>생각을 남길 자리</h2><p>메모 저장과 과거 리딩 보관은 Phase 6에서 구현합니다.</p></div>
        <div className="empty-result"><span>JOURNAL · PHASE 6</span><p>현재 결과를 브라우저에 저장하지 않습니다. 저장소가 준비되기 전까지 민감한 정보나 손바닥 원본을 보관하지 않습니다.</p></div>
      </section>

      <section className="narrative-section">
        <div className="section-heading"><p className="eyebrow">AI COUNSEL</p><h2>결과를 한 편의 상담문으로</h2><p>AI는 계산하지 않습니다. 위에서 확정된 계산값과 규칙 근거, 이름과 질문만 자연스러운 문장으로 연결합니다.</p></div>
      ...
      <aside className="limitations"><b>읽기 전에</b><p>{fullReading.interpretation.limitations.join(" ")}</p></aside>
    </>}

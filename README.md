...
- Palm 촬영 가이드와 왼손·오른손 입력 상태 UI
- 구조화된 통합 결과 화면

현재 Name Engine은 개인화와 한자 입력 여부만 구분하는 mock입니다. Palm 화면은 사진을 만들거나 저장하지 않는 UI 단계이며, 카메라·Palm Analyzer·Journal·PWA는 단계적으로 구현 중입니다. 음력 변환, 신강·신약, 용신·희신, 대운은 방법론 결정 전까지 의도적으로 지원하지 않습니다. 자세한 계산 범위와 한계는 `docs/`를 참고하세요.
